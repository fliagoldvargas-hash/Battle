...
export const solanaExplorerAddress = (address, network) => {
  ...
  return `https://solscan.io/account/${encodeURIComponent(address)}${explorerQuery(network)}`
}
...
export const solanaExplorerTransaction = (signature, network) => {
  ...
  return `https://solscan.io/tx/${encodeURIComponent(signature)}${explorerQuery(network)}`
}
